...
AlertAthlete is implemented with a battery-operated RP2040 microcontroller, a batteryless e-ink display, and real-time clock (RTC). MicroPython code runs through deep sleep cycles with the RTC to periodically download environmental forecasts from the National Oceanic and Atmospheric Administration (NOAA) and OpenWeatherMap. Then, it shows the forecasts and suggested precautions on the e-ink display. AlertAthlete allows athletes to take the necessary precautions (e.g. taking breaks in the shades, extra water and sunscreen), as well as help determine whether the on-going practice/event should be cancelled. 
